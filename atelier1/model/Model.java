...
public class Model implements BoardGame<Coord> {
	...
	private boolean isThereMaxOnePieceOnItinerary(Coord toMovePieceCoord, Coord targetSquareCoord) {
		...
		List<Coord> pieces = implementor.getCoordsOnItinerary(toMovePieceCoord, targetSquareCoord);
		
		if (pieces != null) {
			if(pieces.size() == 1 
					&& implementor.getPieceColor(toMovePieceCoord) != implementor.getPieceColor(pieces.get(0)) 
					&& implementor.getPieceColor(pieces.get(0)) != null) {
				isThereMaxOnePieceOnItinerary = true;
				System.out.println("Piece à bouger : " + implementor.getPieceColor(toMovePieceCoord) + " Piece à prendre : " + implementor.getPieceColor(pieces.get(0)) + "Taille de la liste : "+ pieces.size() + "Premier élément de la liste : "+ pieces.get(0));
			}
	...
	private Coord getToCapturePieceCoord(Coord toMovePieceCoord, Coord targetSquareCoord) {
		...
		if( pieces != null){
			if(pieces.size() == 1) {
				toCapturePieceCoord = pieces.get(0);
